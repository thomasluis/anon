...
bool CMasternodeSync::CheckNodeHeight(CNode *pnode, bool fDisconnectStuckNodes)
{
    ...
    if (!GetNodeStateStats(pnode->id, stats) || stats.nCommonHeight == -1 || stats.nSyncHeight == -1)
        return false; // not enough info about this peer
...
bool CMasternodeSync::IsBlockchainSynced(bool fBlockAccepted)
{
    ...
    if (fBlockAccepted)
    {
        ...
        if (!IsSynced())
        {
            ...
            if (fDebug)
                LogPrintf("CMasternodeSync::IsBlockchainSynced -- reset\n");
    ...
    if (fDebug)
        LogPrintf("CMasternodeSync::IsBlockchainSynced -- state before check: %ssynced, skipped %d times\n", fBlockchainSynced ? "" : "not ", nSkipped);
