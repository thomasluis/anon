...
int CMasternodeMan::CountMasternodes(int nProtocolVersion)
{
    ...
    BOOST_FOREACH (CMasternode &mn, vMasternodes)
    {
        if (mn.nProtocolVersion < nProtocolVersion)
            continue;
        nCount++;
    }
...
int CMasternodeMan::CountEnabled(int nProtocolVersion)
{
    ...
    BOOST_FOREACH (CMasternode &mn, vMasternodes)
    {
        if (mn.nProtocolVersion < nProtocolVersion || !mn.IsEnabled())
            continue;
        nCount++;
    }
...
std::string CMasternodeMan::ToString() const
{
    ...
    info << "Masternodes: " << (int)vMasternodes.size() << ", peers who asked us for Masternode list: " << (int)mAskedUsForMasternodeList.size() << ", peers we asked for Masternode list: " << (int)mWeAskedForMasternodeList.size() << ", entries in Masternode list we asked for: " << (int)mWeAskedForMasternodeListEntry.size() << ", masternode index size: " << indexMasternodes.GetSize() << ", nDsqCount: " << (int)nDsqCount;
