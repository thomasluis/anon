...
CMasternode::CMasternode(CService addrNew, CTxIn vinNew, CPubKey pubKeyCollateralAddressNew, CPubKey pubKeyMasternodeNew, int nProtocolVersionIn) : vin(vinNew),
                                                                                                                                                    addr(addrNew),
...
CMasternode::CMasternode(const CMasternode &other) : vin(other.vin),
                                                     addr(other.addr),
...
CMasternode::CMasternode(const CMasternodeBroadcast &mnb) : vin(mnb.vin),
                                                            addr(mnb.addr),
...
void CMasternode::Check(bool fForce)
{
    ...
    if (!fForce && (GetTime() - nTimeLastChecked < MASTERNODE_CHECK_SECONDS))
        return;
...
bool CMasternodeBroadcast::CheckOutpoint(int &nDos)
{
...

    {
        ...
        {
            LogPrint("masternode", "CMasternodeBroadcast::CheckOutpoint -- Masternode UTXO should have 5000 ANON, masternode=%s\n", vin.prevout.ToStringShort());
            return false;
        }
    ...
    // should be at least not earlier than block when 5000 ANON tx got nMasternodeMinimumConfirmations
